# 软件测试课程

《软件测试从零基础到初级测试工程师》是一套面向软件测试零基础学习者的系统教程。

目标：从零基础学习测试思维、需求分析、测试用例、Web 测试、HTTP、DevTools、Linux、SQL、接口测试、Postman、Python、pytest，并完成可展示的 MiniShop 个人软件测试实践项目。

## 当前进度

- 课程版本：v1.2
- 总章节：22 章
- 当前状态：第 1～5 章及第 7 章已完成审查、修正并发布
- 第 1 章严格复审评分：98/100
- 第 2 章严格复审评分：99/100
- 第 3 章最终评分：99/100
- 第 7 章最终评分：99/100
- 第 4 章最终评分：99/100
- 第 5 章最终评分：99/100
- 发布分支：main

## 重要入口

- [课程主控文件](docs/COURSE_CONTROL.md)
- [v1.2 完整课程大纲](docs/COURSE_OUTLINE_v1.2.md)
- [全局内容质量标准](standards/QUALITY_STANDARD_v1.0.md)
- [第 1 章：软件测试入门](chapters/01-software-testing-intro.md)
- [第 1 章质量审查记录](reviews/chapter-01-review.md)
- [第 2 章：软件研发流程与测试的位置](chapters/02-software-development-process.md)
- [第 2 章质量审查记录](reviews/chapter-02-review.md)
- [第 3 章：软件测试分类体系](chapters/03-software-testing-classification.md)
- [第 3 章质量审查记录](reviews/chapter-03-review.md)
- [第 1～3 章联合严格复审记录](reviews/chapters-01-03-joint-rereview.md)
- [第 4 章：测试需求分析与静态测试](chapters/04-requirements-analysis-and-static-testing.md)
- [第 4 章质量审查记录](reviews/chapter-04-review.md)
- [第 5 章：测试用例设计](chapters/05-test-case-design.md)
- [第 5 章质量审查记录](reviews/chapter-05-review.md)
- [第 7 章：Web 基础](chapters/07-web-basics.md)
- [第 7 章质量审查记录](reviews/chapter-07-review.md)

## 每章制作流程

1. 生成章节正文
2. 按 20 项质量门槛审查
3. 按 100 分标准评分
4. 修正所有问题
5. 重新审查并达到至少 90 分
6. 提交正文和审查记录到 main
7. 再进入下一章

## 目录结构

- chapters/：正式章节正文
- reviews/：逐章质量审查、评分和修正记录
- standards/：主控质量标准和项目规范
- docs/：课程大纲、学习路线和项目说明
- project/：MiniShop 项目材料
- exercises/：练习和答案

## 项目声明

MiniShop 仅作为个人软件测试实践项目，不冒充企业工作经历。课程中的测试结果、Bug 和项目材料必须能够在仓库中找到对应依据。
