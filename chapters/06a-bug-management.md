...
关系通常是：根因 → 错误 → 缺陷 →（可能）失效。人的错误可能把缺陷引入需求、设计、代码、配置或测试工作产品；缺陷在特定条件被执行或使用时，才可能表现为失效。不是每条失效都能追到单一根因。

因此：

- 测试人员直接观察到的常常是**失效**；
- 缺陷报告的目的是帮助团队定位并处理**缺陷**；
- **根因**分析用来减少同类再发，不要把未验证的假设写成事实。
...
本课程中“Bug”和“缺陷”视为同一类工作项的常用叫法。口头交流可以说 Bug；书面和面试中应能切换到根因、错误、缺陷、失效这些更准确的词。
...
- 回归测试：检查这次修改是否影响加购、改数量、创建订单等关联区域。
...
| 等级 | 课程定义 | 例子 |
| --- | --- | --- |
| S1 致命 | 核心流程不可用，或导致严重数据/资金/权限错误，且没有可接受的变通方法 | 支付成功但重复扣款（行业后果教学示例，v1.0 无支付）；所有用户无法登录 |
| S2 严重 | 主要功能错误，明显影响关键任务，变通方法很差或仅少数人能绕过 | 购物车可把数量改为超过库存并超卖下单，成功 `POST /api/orders` 得到第二个 id |
...
| 组合 | 理解 | MiniShop 例子 |
| --- | --- | --- |
| 高严重程度 + 高优先级 | 影响大，且必须尽快修 | 库存为 10 时购物车仍可改成 11 并生成订单（v1.0 若发生） |
...
| 偶现 bug | 连续快速点击 `POST /api/orders` 时，约 3/10 次得到第二个 id |
...
下面是**写法示范**，不是 MiniShop v1.0 的当前行为。仓库实现会拒绝 `qty=11`（实操 5-1）。作业请写空搜索 BUG-001，不要交这一份。失败后数量是否保持原值，PRD 未冻结，示例里的「保持为 1」只是示范怎么写预期，不要抄进项目用例。
...
3. 保存或触发数量提交。
4. 观察页面数量和提示。v1.0 没有价格/小计字段；若教学草案里出现小计，标明那是教学字段。

## 预期结果
提交失败；数量保持为 1；出现库存不足或超限提示。

## 实际结果
数量显示为 11；无失败提示。刷新后数量仍为 11。
...
可选（不挡本章过关）：用 Jira 免费云或禅道个人版把 `bugs/BUG-001.md` 建一次单，走「提交 → 指派 → 验证仍失败则保持开放」。把工单 key 和状态写进 `exercises/chapter-06-minishop-bug.md`。不要把某次按钮路径写成全球标准，也不要把课程仓库截图冒充公司 Jira。
...
本节与 [实操 6-1](../practice/06-bug-report/README.md) **同一套验收**（📖）。先跑实操 1-1 复现空搜索，再自己写报告。不要另写一份完整单。

写 **空搜索 BUG-001**。6.5 的 qty=11 是写法示范：v1.0 **会拒绝** 11，不要交那份，也不要在 MiniShop 上把「改成 11 成功」当成当前缺陷。计划摘要放到 06B。
...
修正：完整处理见 06B。还要看范围、未测项、未关闭缺陷、环境和剩余风险。
...
测试计划、入口/出口、未关闭缺陷能否上线：见 [06B](06b-test-management.md)。
