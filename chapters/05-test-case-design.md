...
MiniShop 下单基本流程：登录 → 选择商品 → 加入购物车 → 确认订单 → 提交 → 支付。

备选或异常包括：优惠券、地址切换、库存不足、价格变化、支付取消、重复提交、网络中断。
...
| 场景 ID | 流程 | 关键检查 |
| --- | --- | --- |
| SC-ORDER-01 | 正常下单并支付 | 订单、库存、金额和支付状态一致 |
| SC-ORDER-02 | 使用有效优惠券 | 优惠规则和实付金额正确 |
| SC-ORDER-03 | 提交前库存不足 | 阻止下单并明确提示 |
| SC-ORDER-04 | 重复点击提交 | 不产生非预期重复订单 |
| SC-ORDER-05 | 支付取消 | 订单状态符合已确认规则 |
| SC-ORDER-06 | 支付响应超时 | 可查询最终状态且不重复扣款 |

本章不冻结订单状态名称；正式状态将在 MiniShop 项目规范中统一。
...
4. 三项都为 Y 时抵扣。任一关键条件不满足则不抵扣，但不同失败原因和优先级若不同，需要保留相应规则；只有动作完全相同且条件确实无关时才能合并。
5. 参考本章状态图：正常达到失败阈值进入锁定；锁定满足解锁条件回到正常；正常或锁定可被管理员禁用；禁用状态正确密码仍不能登录。具体规则以需求为准。
6. 基本场景：登录后选择有库存商品并成功下单。异常可包括库存不足、重复提交、支付取消或网络超时，任写三个并给出检查点。
...
本章没有 Python、SQL、curl、JSON 或 pytest 代码。Mermaid 因果图和状态图为教学模型；手机号、验证码、密码及优惠券规则均为本章局部教学基线，不代表 MiniShop 最终 PRD 已冻结。
...
下一章进入第 6 章《Bug、缺陷管理与测试管理基础》。你会把用例执行中发现的问题写成可复现、可沟通、可跟踪的缺陷，并学习严重程度、优先级、测试计划、风险和测试报告。
