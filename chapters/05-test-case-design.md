...
- 理解状态迁移测试（锁定/禁用为通用练习系统，不算 MiniShop 过关）；
- 使用错误推测补充系统化技术；
...
图和下面这张 Markdown 是同一张业务：MiniShop v1.0 注册占用表（PRD `R-REG`）。图中「是 / 否 / —」对应表里的 Y / N / `-`。优惠券、结算确认页放到 5.10「通用练习系统」，禁止写进第 19 章用例。

先用 MiniShop v1.0 注册。实现校验顺序：非法手机号 400 → 非法密码 400 → 已占用 409。成功时 201，JSON 有 `phone`、无 `token`。
...
`-` 表示该条件在本规则下不影响结果。R2 在手机号已非法时不再看密码和占用，所以标 `-` 并得到 400，而不是 409。三条件共八种组合，合并后四条有效规则都应至少由一条测试覆盖。
...
本节是通用练习系统，**不是** MiniShop v1.0，不算过关。禁止把优惠券、确认页写进第 19 章用例。
...
这个例子经过规则合并，没有机械列出所有八种组合。不同失败原因若提示不同，不要合并成一条“失败”。

### 购物车结算与确认页（教学模型）

只有已登录、购物车非空、全部商品可售且库存充足时，才允许进入订单确认页。v1.0 没有确认页、没有结算金额；创建订单是 `POST /api/orders`，成功只返回 `id`。不要把「进入确认页」写进 MiniShop 用例。
...
5.10 的优惠券例子（通用练习系统，**不是** v1.0）可表示为：
...
MiniShop v1.0 可执行下单路径：登录 → 选 SKU → 设合法 qty → 创建订单（`POST /api/orders`）→ 得到 `id`。没有确认页、没有支付、没有优惠券、没有订单 `status`。

备选仍完成同一目标：例如先把 qty 从 1 改到 10 再下单（仍得到 `id`）。异常可用库存不足（`qty=11`）、未登录、重复提交。支付取消、优惠券只作为场景法技术示例，放到下一节的通用练习系统表，不要写进第 19 章项目用例。

场景法适合覆盖端到端业务，但不自动覆盖所有输入边界。通常要与等价类、边界值和判定表组合。

## 5.13 场景法实战 ⭐⭐⭐

过关只抄下面这张 **v1.0 可执行**表。没有确认页、没有支付、没有订单状态机。

| 场景 ID | 流程 | 关键检查 |
| --- | --- | --- |
| SC-ORDER-01 | 正常创建订单 | 返回订单 `id`，Body 不含 `status` |
| SC-ORDER-03 | 提交前库存不足（`qty=11`） | 阻止下单；接口 400 |
| SC-ORDER-04 | 重复点击提交 | 记录两次响应：v1.0 创建订单默认不幂等，两次成功得到两个 `id` 符合当前契约；若正式需求改为只允许一笔，再按防重失败报缺陷 |
| SC-ORDER-06 | 未登录创建订单 | HTTP 401；不以状态名字段为准 |

MiniShop v1.0 **不定义**待支付/已发货等状态名；创建成功只保证返回 `id`。网络中断后重试仍可观察「能否查到已创建的 `id`」，不要发明 `status`。

### 加分：通用练习系统场景（禁止抄进 MiniShop）⭐⭐

ID 前缀改成 `SC-GEN-`，避免和 v1.0 行焊在一起。

| 场景 ID | 流程 | 关键检查 |
| --- | --- | --- |
| SC-GEN-COUPON | 使用有效优惠券 | 优惠规则和实付金额正确；**不是** v1.0 |
| SC-GEN-PAY | 支付取消 | 不冻结、不发明订单状态名；**v1.0 无支付、无状态机** |
...
通用练习系统（**不是** MiniShop v1.0）的登录账户状态：正常、锁定、禁用。PRD 无锁定、无禁用。这是教学模型，不算 MiniShop 过关。v1.0 没有锁定、禁用、订单状态机。若要用仓库体会「同一事件、不同状态」：未登录改购物车 → 401；已登录后同样请求可以成功。不要为了凑技术去发明待支付或锁定阈值。
...
本节是教学模型，**不是** MiniShop v1.0，不算过关。不要把锁定、禁用、待支付写进项目用例包。
...
本章过关物只有下面两件，都是 v1.0。练习 10 就是第 2 件，不要另交一套 8 条。

1. `python3 practice/run.py 5-1` 与 `python3 practice/run.py 5-1 --check` 通过。
2. 同一份 `exercises/chapter-05-minishop-test-cases.md`。
...
1. 需求基线：抄 PRD 的 `R-PHONE` / `R-PASS` / `R-REG` / `R-CART-10`，并写明**没有**验证码、优惠券、结算页、锁定、订单状态机；
2. 实操 5-1 的观察：`qty=10` 通过、`qty=11` 拒绝；
3. 等价类表、边界值表（密码尺用 8 和 16，不是 8 和 20）；
4. 一张判定表或场景表（注册占用/未占用，或购物车已登录/未登录即可）；
5. 不少于 8 条可在 MiniShop 上执行的用例；
6. 需求—测试点—用例追踪；
7. 一次用例评审记录（可自评）。

完成标准：跑过 5-1（含 `--check`）；能解释等价类、边界值，以及判定表**或**场景法（占用/登录即可）。状态迁移、验证码、优惠券、锁定、确认页是可选加分，标题不得叫 MiniShop，不算过关。P0 能指向仓库真实规则。
...
> 场景法围绕用户目标和完整流程；状态迁移关注当前状态、事件和历史对下一行为的影响。下单流程适合场景法。账号锁定和验证码生命周期适合状态迁移，但那是通用练习系统，**不是** MiniShop v1.0，两者也可以组合。
...
4. 为**通用练习系统**（不是 MiniShop v1.0）的“已登录、金额达标、优惠券有效才抵扣”补全判定表并说明是否可合并规则。
...
10. 过关书面作业与「MiniShop 工作实战」是**同一份**文件，不要另写一套 8 条：完成 `exercises/chapter-05-minishop-test-cases.md`（等价类 + 边界值 + 判定表或场景法；≥8 条 v1.0 可执行用例）。
...
4. 可写成四条规则（与 5.10 优惠券表同类）：已登录/金额≥100/券有效均为 Y 才抵扣；券无效、金额不足、未登录分别拒绝。未登录时金额和券可标 `-` 并合并。不同失败原因若提示不同，不要合并成一条“失败”。这是通用练习系统技术示例，**不是** MiniShop v1.0。
...
10. 练习 10 就是工作实战那一份 `exercises/chapter-05-minishop-test-cases.md`，不是第二套 8 条。需求基线必须是 PRD；至少覆盖等价类、边界值，以及判定表或场景法（注册占用或登录即可）；包含前置、数据、步骤和预期。状态迁移/锁定不算过关。验证码/优惠券只能出现在单独的「通用练习系统」节。
...
- [ ] 我理解因果图与判定表的关系
- [ ] 我能覆盖基本、备选和异常流程
- [ ] （加分）我能为通用练习系统建立状态图或状态表；不要求画进 MiniShop
- [ ] （加分）我能说明合法与禁止的状态迁移；锁定不算过关
- [ ] 我能使用有依据的错误推测补充测试
...
1. 练习 1～9 至少完成 8 题；
2. 能不看正文解释等价类、边界值、判定表和场景法；状态迁移能说明「当前状态会影响同一事件的结果」，不要求把锁定写进 MiniShop；
3. 完成过关物（见「MiniShop 工作实战」）：`python3 practice/run.py 5-1` 与 `--check`，以及同一份 `exercises/chapter-05-minishop-test-cases.md`（练习 10 就是它，不要另交一套 8 条）。
