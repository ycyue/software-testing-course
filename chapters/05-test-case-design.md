...
| ID | TC-REG-001 |
| 标题 | 使用未注册的合法手机号完成注册 |
| 依据 | R-REG-01、TP-REG-01 |
| 优先级 | P0（课程约定） |
| 前置条件 | 手机号 `13800138000` 未注册；验证码有效 |
| 数据 | 手机号 `13800138000`；密码 `Test1234` |
| 步骤与预期 | 1. 打开注册页 → 页面可用；2. 输入合法数据 → 字段正确接收；3. 单击提交 → 只提交一次并注册成功；4. 查询结果 → 只创建一个用户且跳转符合需求 |
| 后置/清理 | 在授权测试环境删除或重置本用例创建的测试账户；如需保留证据则使用专用账号并标记状态 |
...
| 编号 | 等价类 | 有效性 | 数据 |
| --- | --- | --- | --- |
| EP-01 | 恰好 11 位数字 | 有效 | `13800138000` |
...
真实手机号规则可能比“11 位数字”复杂，本例只用于学习技术。正式 MiniShop 规则必须在 PRD 基线化后更新。

## 5.7 边界值分析 ⭐⭐⭐

边界值分析（Boundary Value Analysis，BVA）用于有序分区，重点检查边界及其相邻值，因为范围判断容易出现 `<`、`<=`、偏移或漏判错误。

![密码长度尺：7 拒绝，8 和 20 接受，21 拒绝](assets/diagrams/ch05-boundary.png)
...
教学规则：密码长度 8～20 个字符，按 Unicode 字符数量计数。
...
“字符数量”必须由需求明确。如果前端按字符计数、后端按字节限制，中文、Emoji 等输入可能产生不一致。
...
教学规则：只有已登录、购物车非空、全部商品可售且库存充足时，才允许进入订单确认页。
...
以下规则**仅用于本章练习设计技术**。短信验证码、8～20 位密码、隐私政策勾选都不是 MiniShop v1.0 范围（v1.0 注册是手机号 + 8～16 位字母数字密码，无短信）。第 19 章项目用例必须跟 PRD，不要把本表抄进简历。

1. 手机号必填且为 11 位数字；
2. 手机号不得已注册；
3. 验证码为 6 位数字且在有效期内（状态迁移练习用，项目无此接口）；
4. 密码长度 8～20 个字符（本章边界练习；v1.0 为 8～16 且须同时包含字母和数字）；
5. 用户必须同意隐私政策；
6. 全部满足时创建一个账户。
...
| 手机号、验证码、密码格式 | 等价类 |
| 长度 11、6、8～20 | 边界值 |
| 手机号未注册、验证码有效、同意政策 | 判定表 |
| 获取验证码到提交注册 | 场景法 |
| 验证码未发送、有效、过期、已使用 | 状态迁移 |
| 重复点击、空格、多标签页 | 错误推测 |
...
| ID | 标题 | 技术 | 优先级 |
| --- | --- | --- | --- |
| TC-REG-001 | 合法数据成功注册 | 场景/判定表 | P0 |
| TC-REG-002 | 10 位手机号被拒绝 | 等价类/边界值 | P1 |
| TC-REG-003 | 11 位手机号可进入后续校验 | 边界值 | P1 |
| TC-REG-004 | 12 位手机号被拒绝 | 等价类/边界值 | P1 |
| TC-REG-005 | 已注册手机号不能再次注册 | 等价类 | P1 |
| TC-REG-006 | 过期验证码被拒绝 | 状态迁移 | P1 |
| TC-REG-007 | 未同意隐私政策不能注册 | 判定表 | P1 |
| TC-REG-008 | 快速重复提交只创建一个账户 | 错误推测 | P1 |

完整设计还应覆盖密码边界、验证码格式、服务异常、页面刷新、失败后的数据保持和权限风险。
...
```text
exercises/chapter-05-registration-test-cases.md
```
...
完成标准：每种核心技术至少有一项可解释的应用；所有用例都有明确预期；P0/P1 用例能够在发布候选版本中优先执行；没有把课程假设冒充正式项目规则。

最小追踪表示例：

| 需求 ID | 测试点 ID | 用例 ID | 技术 | 状态 |
| --- | --- | --- | --- | --- |
| R-REG-01 | TP-REG-PHONE-01 | TC-REG-002～004 | 等价类/边界值 | 已设计 |
| R-REG-05 | TP-REG-PRIVACY-01 | TC-REG-007 | 判定表 | 已设计 |
...
5. 为 MiniShop 登录画出正常、锁定、禁用三个状态的关键迁移。
6. 为下单流程写一个基本场景和三个异常场景。
7. 列出五个有来源的错误推测，并说明来源。
8. 判断：P0 在所有公司都代表最高优先级。为什么？
9. 将“点击按钮，检查正常”改写为可执行步骤和明确预期。
10. 选择注册、登录或购物车，使用至少三种技术设计 10 条测试用例。
...
5. 参考本章状态图：正常达到失败阈值进入锁定；锁定满足解锁条件回到正常；正常或锁定可被管理员禁用；禁用状态正确密码仍不能登录。具体规则以需求为准。
6. 基本场景：登录后选择有库存商品并成功创建订单（检查返回 `id`、无 `status`）。异常可包括库存不足、重复提交（v1.0 默认不幂等，两次成功两个 `id`）、网络超时；支付取消只作为技术示例，不要写成 MiniShop v1.0 用例。
...
9. 示例：前置为合法数据已填写；单击“注册”；预期提交一次请求，成功时创建一个用户并按需求跳转，失败时显示规定原因且不创建用户。具体结果必须来自需求。
10. 开放题。至少明确需求基线，使用三种技术，包含前置、数据、步骤和预期，并能说明每条用例覆盖的分区、边界、规则、场景或状态。
...
本章没有 Python、SQL、curl、JSON 或 pytest 代码。Mermaid 因果图和状态图为教学模型；手机号、验证码、8～20 位密码、优惠券和支付场景均为本章局部教学基线。MiniShop v1.0 无短信、无优惠券、无支付、无订单状态机；正式规则以第 19 章 PRD 为准。
