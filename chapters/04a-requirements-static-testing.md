...
| 测试用例（Test Case） | 用什么前置条件、数据、步骤和预期结果检查？ | 库存 10，输入 11，提交后应拒绝（失败后原值是否保持，PRD 未写则待确认） |
...
已确认规则只写 PRD `R-CART` / `R-CART-10` 已经有的三句（正式契约以 `project/minishop/docs/PRD.md` 为准）：

> 登录用户可以修改自己购物车中的数量；数量必须是正整数且不得超过该 SKU 当前可售库存。教学库存为 10 时，`qty=10` 允许、`qty=11` 拒绝。
...
下面这些 **PRD 未写，记待确认，不要当成已冻结预期**：失败如何提示、失败后原值是否保持、小计/总价、商品下架。教学示例若要练「小计重算」，单独标明通用练习系统，不要写进 MiniShop 用例。
...
下面三处说明是否存在矛盾？你应该怎样处理？（这是虚构的矛盾材料，**不是** MiniShop PRD。v1.0 密码是 8～16 且须含字母和数字。）
...
- 数据：删除后刷新页面，条目保持一致（总价仅当文档有金额字段；v1.0 无）；
- 状态：商品已经下架时执行删除。
