...
本章路径、字段名和响应形状是**教学约定**，不是仓库级 MiniShop 正式 OpenAPI。订单状态名称仍不冻结。正式契约以后续项目文档为准。
...
| 等于库存 | `{"sku":"SKU-DEMO-001","qty":10}` | 按规则应允许 |
...
审查用教学服务时：`qty=1` 返回 `200` 且回显数量；`qty=11` 返回 `400` 且 JSON 说明超过库存。这是**教学服务行为**，不是已冻结的 MiniShop 错误码表。
