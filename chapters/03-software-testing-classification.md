...
- 为 MiniShop 的具体场景选择合适的测试分类，并保存一张分类卡片；
- 避免把不同维度的术语放在一起做错误比较。
...
第 2 章 2.7 已定义。本轴只回答这一轮为什么测。MiniShop 的冒烟集合可能包括：首页能打开、用户能登录、商品列表能加载、能更新合法数量、能创建订单。冒烟通过不代表系统质量已经合格。

## 3.7 确认测试与回归测试（复习第 2 章）
...
![探索性测试带着目标和时间盒，不是随便点](assets/diagrams/ch03-exploratory.png)
...
从 MiniShop 选择一个场景，例如登录、更新购物车数量或创建订单（不要写加入购物车/结算/下架），建立一张分类卡片：
