...
| 测试方法视角 | 是否依据内部结构设计测试？ | 黑盒、白盒，以及实践中的灰盒 |
...
静态测试（Static Testing）不执行被测对象，而是通过评审、静态分析等方式检查需求、设计、代码或其他工作产品。例如在 MiniShop 需求评审中发现“退款后优惠券是否返还”没有定义。

动态测试（Dynamic Testing）需要执行被测对象并观察行为。例如启动 MiniShop，提交一笔使用优惠券的订单，再检查页面、接口和数据结果。

静态和动态描述的是是否执行被测对象，并不等于“手工”和“自动化”。静态分析可以由工具支持，动态测试既可以手工执行，也可以自动执行。第 4 章将主讲静态测试和需求评审，本节只确定它在分类体系中的位置。
...
非功能测试关注系统“以什么质量完成工作”。它可以涉及性能效率、兼容性、可靠性、安全性、易用性等质量目标。
...
MiniShop 示例：单独验证“计算优惠金额”函数在满减边界上的结果。

### 集成测试（Integration Testing）

关注组件、模块、服务或外部系统之间的接口和交互。

MiniShop 示例：订单服务调用库存服务扣减库存后，订单和库存状态是否一致；支付回调能否正确更新订单。
...
MiniShop 示例：从注册、登录、选商品、加入购物车、提交订单到模拟支付，验证完整购物流程。

### 验收测试（Acceptance Testing）

从业务和用户需要出发，判断系统是否满足验收标准，是否具备被接受或交付的条件。业务方、客户、产品代表或用户可能参与，具体责任以项目约定为准。

MiniShop 示例：产品负责人依据已确认的验收标准，验证优惠券功能能否支持计划中的促销活动。
...
冒烟测试（Smoke Testing）是一组范围较小、优先级较高的测试，用于快速判断构建版本的关键能力是否基本可用，是否值得继续进行更深入测试。
...
冒烟通过不代表系统质量已经合格，只代表当前版本没有在这些关键检查上立即失败。
...
开发修复某个缺陷后，重新执行与该缺陷直接相关的测试，确认原问题是否已经修复。有些团队也会口头称为“缺陷验证”或“复测”。

### 回归测试（Regression Testing）

修改可能影响原本正常的功能，因此需要测试未直接修改但可能受影响的区域，确认没有引入新的回归问题。

例子：MiniShop 修复“购物车数量能超过库存”的缺陷。

- 再次执行“库存 10 件时不能设置为 11 件”：确认测试；
- 检查购物车删除、结算、库存扣减和订单金额：回归测试。

同一轮工作中可以先做确认测试，再做有针对性的回归测试。回归范围应根据修改内容和风险决定，不等于每次机械执行全部用例。
...
例如“搜索很快”不可测试；“在约定环境和负载模型下，95% 搜索请求响应时间不超过目标阈值”才更接近可验证目标。
...
1. 开发用单元测试验证商品状态判断分支；
2. 用集成测试验证商品服务与购物车服务的交互；
3. 测试人员从页面执行黑盒功能测试；
4. 修复后重测原失败场景，属于确认测试；
5. 检查正常商品加购、购物车结算和库存，属于回归测试；
6. 将稳定的核心场景加入自动化回归；
7. 发布候选版本先跑关键冒烟集合。

这里没有哪一个名称能单独描述全部工作。专业表达应该包含维度，例如：“对购物车服务执行集成级功能测试，并在修复后完成确认测试和风险相关回归测试。”
...
> 我会按维度分类。按级别有单元、集成、系统和验收测试；按质量目标有功能和各类非功能测试；按是否依据内部结构可讨论黑盒和白盒；按执行方式分手工与自动化；按变更后的目的还会区分确认和回归测试。不同维度可以组合，不能混为互斥分类。

### 问题 2：确认测试和回归测试有什么区别？

先给结论，再举例：确认测试检查原缺陷是否修好；回归测试检查修改是否破坏其他原本正常的功能。购物车库存缺陷修复后，重测原场景是确认，继续检查结算和库存扣减是回归。
...
需求评审发现“退款后优惠券是否返还”没有定义，这属于静态还是动态测试？如果之后运行系统验证退款结果，又属于哪一种？
...
6. 不能。目标环境可能包括其他浏览器、版本、操作系统、设备和网络条件，应依据兼容矩阵判断。
7. 依次为单元/组件测试、集成测试、系统测试、验收测试。
8. 需求评审没有执行被测系统，属于静态测试；运行系统并观察退款结果，属于动态测试。
