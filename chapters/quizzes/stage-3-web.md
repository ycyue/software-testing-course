...
9. MiniShop v1.0 登录成功时，你应在 Network 里看到哪两类凭证痕迹？（第 19 章冻结后）
...
4. safe：不应产生除取信息以外的服务器副作用。幂等：同一请求做多次，效果与做一次相当。不能用“安全/不安全”替换这两个词，更不能说 GET 不安全、POST 安全。
5. 401 未通过认证（无 token 下单）；403 已认证但没权限（普通用户打 `/api/admin/products`）。
