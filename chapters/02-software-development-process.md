...
- 使用最小 Git 工作流保存一次文档修改，并在自己的仓开一条 PR。
...
修复“购物车可超过库存”后，重新执行原复现步骤属于确认测试；再检查合法改数量、登录和创建订单（Body 无 `status`）属于相关回归测试。二者可能在同一次测试会话中执行，但目的不同。
...
> 这条数量规则与第 19 章 PRD `R-CART` 同方向，本章先当教学假设。

### 各阶段测试活动

类型：📖 书面。把表抄到 `exercises/ch02-lifecycle.md` 后自己填，不要照抄场景导入。一行示范已给出，其余格子留空。

验收：五格以上有内容；确认和回归不要写反；不要写结算、下架、验证码。

| 阶段 | 测试问题或动作（自己填） | 产出 |
| --- | --- | --- |
| 需求 | （示范）库存 10 时改成 11 谁来拦？失败提示写了吗？ | 评审问题清单 |
| 设计 |  |  |
| 开发 |  |  |
| 提测 |  |  |
| 执行 |  |  |
...
在自己的练习目录里做，不要 `git add chapters/` 教材。第一帧：

```bash
mkdir -p exercises
printf 'SDLC notes\n' > exercises/notes.md
cd exercises
git init          # 若你已经在课程仓库里练习，跳过 init，只 add 自己的 notes
git status        # 应看到 Untracked: notes.md（或 exercises/notes.md）
git add notes.md
git status
git diff --staged
...
若 `git add notes.md` 报 pathspec，说明文件还不在当前目录——先 `ls`，不要改去 `git add .`。
...
1. **先有一个你拥有的远端仓。** 在 GitHub 或 Gitee 上 fork 本课程仓库，或新建空仓（建议勾选 README，便于 clone）。然后 clone **你的**仓，不要在课程仓库子目录里再 `git init`。
2. **建分支**，不要在 `main` 上直接改：

```bash
git switch -c practice-ch02
# 若提示 unknown switch：git checkout -b practice-ch02
```

3. **一次 commit。** 只改 `exercises/` 里你的笔记或作业，沿用上一小节的 `status` → `add` → `commit`。不要 `git add chapters/`，不要提交 `.venv`、密码或密钥。
4. **推到你的远端并开 PR：**

```bash
git push -u origin practice-ch02
```

到网页上，对**你自己的仓库**把 `practice-ch02` 合入 `main`，新建 Pull Request。fork 后点「Compare & pull request」时，基座常会默认指向课程原仓库——改成你自己的 `main`。描述里写：**MiniShop 是个人实践，不是任职项目，不是独立开发。**

能打开这条 PR 链接，就说明你会「分支 + 一次提交 + 请求合入」。其余 Git 内部机制本章不展开。
...
在自己的练习仓库修改一个 Markdown 文件，依次使用 `status`、`diff`、`add`、`diff --staged` 和 `commit`，保存命令输出或截图作为证据。不要推送到不属于你或未经授权的远端。有 GitHub/Gitee 账号时，在自己的仓建分支、推送并开一条 PR，描述写明 MiniShop 是个人实践。
...
9. 结果因环境不同而不同。通过标准：能解释五条命令作用，提交仅包含预期文件，且没有误推送敏感内容或无权限远端。有远端仓时，另有一条自己仓库上的 PR，描述标明 MiniShop 是个人实践。
...
- [ ] 我能写出包含范围与风险的测试结论
- [ ] 我能完成一次最小 Git 本地提交
- [ ] 我能在自己的仓用分支开一条 PR（描述写 MiniShop 是个人实践）
...
4. 有条件使用 Git 时完成练习 9（含自己仓上的 PR）；暂时没有环境时，至少能解释五条命令和「分支 → 提交 → PR」，不盲目执行。
...
本章只有 Git 命令实操，不包含 Python、SQL、curl、JSON 或 pytest。Git 命令已依据官方文档核对；学习者应只在自己的练习仓库执行，PR 只开在自己拥有的远端仓。Mermaid 图为教学模型，不表示所有组织的固定流程。
