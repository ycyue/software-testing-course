...
MiniShop 收到一个需求：“购物车中的商品数量可以修改。”它不会直接从一句话跳到线上功能。团队通常要经历以下活动：
...
STLC（Software Testing Life Cycle，软件测试生命周期）是教学和行业中常用的测试活动组织模型，通常包括测试分析、计划、设计、实现、执行、完成和维护等活动。
...
V 模型强调开发工作产品与相应测试活动之间的关系：需求要考虑验收，系统设计要考虑系统测试，详细设计和组件要考虑更低层级测试。

它的价值不是字母形状，而是提醒团队尽早思考“以后怎样验证”。V 模型也不代表所有测试只能在右侧一次性执行。
