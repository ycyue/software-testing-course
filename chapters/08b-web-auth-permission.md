...
1. 用账号 A 登录并创建一笔订单，记下订单 `id`；
2. 换账号 B 登录，带 B 的 token 请求 `GET /api/orders/{id}`；
3. 预期：**403**，Body 不含该 id；
4. 再用普通用户请求 `GET /api/admin/orders`，预期 **403**。

不要把 404 写成 MiniShop 的可接受结果。有的系统用 404 隐藏资源，那是另一份契约。

只在授权环境使用自己创建的测试账号。不要对生产用户或外部系统猜测他人 ID。

页面隐藏按钮不等于没有权限。`/admin.html` 静态页任何人打开都可能是文档 200，这不等于 API 越权成功；还要看 `/api/admin/*` 是否 403。功能测试必须测直接访问接口，而不是只测菜单里看不看得见。
...
1. 功能范围和未测项；
2. 注册/登录/搜索/购物车/权限测试点表（分页、退出若无入口列入未测项）；
...
- 测试点覆盖 v1.0 有的搜索、登录、权限；分页与退出若无入口，写入未测项，不要编步骤；
- 至少有一条未登录或换账号后的权限用例；
- 至少有一条空值或非法输入用例；
- 用例步骤可执行，预期可观察；
- 缺陷标题含对象、条件和实际结果，或明确写清已测范围且未发现失效；
- 当前实现路径以 v1.0 为准（冻结仪式在第 19 章），不要编不存在的订单详情页或可分享购物车 URL；
- 不在报告中粘贴密码或完整 Token。
...
修正：权限以服务器判定为准。直接访问 URL 或打 `/api/admin/*`，普通用户应 403。`/admin.html` 静态页 200 ≠ API 403。见实操 8-1。
...
用户 B 带自己的 token 请求用户 A 的 `GET /api/orders/{id}`，得到 200 和 A 的数据。这属于哪类权限问题？缺陷报告至少还要写哪些上下文？MiniShop 当前预期是什么状态码？
...
6. 横向越权。MiniShop 预期是 **403**，不是 200。报告还应包含两个账号、脱敏后的 `GET /api/orders/{id}`、实际 Body 是否含 A 的数据。只在授权测试账号上复现。实操 8-1 就是这条。
...
第 9 章《计算机网络与 HTTP》，先读 [09A](09a-network-http-semantics.md)。学完第 10 章后做 [阶段测验 3](quizzes/stage-3-web.md)。
