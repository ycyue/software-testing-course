...
- 测试注册、登录和退出，而不是只测登录成功（v1.0 无退出入口则记未测项）。
...
| 问题 | 例子 |
| --- | --- |
| 入口找得到吗？ | 搜索框、登录、「更新数量」「创建订单」是否可见可用 |
| 正常路径通吗？ | 合法搜索、合法登录、合法修改购物车数量 |
| 空值和非法输入呢？ | 空搜索、超长词、错误密码（v1.0 **无**验证码） |
| 状态变了还能对吗？ | 登录后、未登录直接访问（v1.0 **无**下架、无锁定） |
| 数据一致吗？ | 列表与购物车数量（v1.0 **无**详情页、无小计） |
...
| 对象 | 测试时要确认 | MiniShop 例子（v1.0 有的才写 MiniShop） |
| --- | --- | --- |
| 按钮 | 能否聚焦、单击、重复单击；禁用时是否仍可提交 | 「登录」「注册」「更新数量」「创建订单」 |
| 链接 | 跳转目标、新窗口、失效地址 | 后台页 `/admin.html`（一般站点才有「进详情」） |
| 输入框 | 可输入、可粘贴、最大长度、清空 | 手机号、密码、搜索词、数量 |
| 单选/复选 | 互斥或可多选是否符合规则 | v1.0 无规格/协议勾选；一般站点要测 |
| 下拉框 | 默认值、选项完整性、可搜索时的过滤 | v1.0 无排序；一般站点要测 |
| 图片 | 加载失败时的替代信息、点击热区 | v1.0 无商品主图 |
| 提示文本 | 是否来自真实校验，会不会挡住操作 | `qty exceeds stock`、登录失败 |
...
沿用第 4、6 章的教学规则：数量必须是正整数，且不得超过提交时可售库存。测正在跑的 MiniShop v1.0 时以 PRD `R-CART-10` 为准；冻结仪式在第 19 章。
...
分页把长列表拆成多页。缺陷常出现在最后一页、非法页码和筛选条件丢失。MiniShop v1.0 无分页入口：没有入口就写入未测项，不要编步骤。
...
退出是登录测试的一部分，常常被漏掉。MiniShop v1.0 无退出按钮：没有入口就写入未测项，不要编一个不存在的退出步骤。通用站点仍应测：
...
### 错误 1：页面上的 `required` / `maxlength` 等于服务端也会拒绝

修正：这些是浏览器约束，可被绕过。服务端是否拒绝，要看实际保存结果或绕过页面的请求。

### 错误 2：空白搜索只写“不能搜”

修正：要观察有没有发出请求、结果是全量还是空列表、有无提示、URL 里是不是只有空格。MiniShop 空白搜索仍列出三件商品，这是 BUG-001。

### 错误 3：页面空白一定是缺陷

修正：可能是合法空态，也可能是未处理的非法页码。先补观察：总页数、提示、改回有效页是否恢复。

### 错误 4：测完登录就不测退出

修正：退出后访问需登录页、浏览器后退、多标签继续提交，都是登录测试的一部分。

### 错误 5：只看页面文案，不写控件角色

修正：先认 label、按钮、输入类型，再设计步骤。没有对象的用例无法复现。

## 面试角度 ⭐⭐⭐

### 浏览器约束和服务端校验有什么区别？

结论：浏览器约束只挡普通用户的正常操作，不是规则本身。  
示例：登录表单 `required` 挡空提交，用 DevTools 删掉属性或直接打接口仍可能成功。  
边界：两边都要测；只测页面会漏过被绕过的路径。

### 搜索只输入空格，应观察什么？

结论：至少看请求有没有发出、返回什么、页面怎么解释、刷新后是否保持。  
示例：MiniShop 空/空白关键字仍返回三件商品，对应 R-SEARCH 和 BUG-001。  
边界：不要把“不能搜”当成一条完整预期。

### 登录功能测完了吗？

结论：还要测退出和登录态是否还在。  
示例：退出后点后退仍看到账号页，可能只是缓存，还要试一次写操作。  
边界：具体跳转以需求为准，但必须测；v1.0 无退出入口则记未测项，不能默认“有退出按钮就算过”。
...
- [ ] 我知道浏览器约束不等于服务端校验
- [ ] 我能为搜索、分页、上传列出测试点
- [ ] 我能测试注册、登录；退出与分页若无入口则记未测项

## 本章总结

同一功能要分页面观察和服务器判定。浏览器约束不是规则；空白和空态都要写出可观察预期。完整的 Web 测试包、登录态和权限放到 08B。
