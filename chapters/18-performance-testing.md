...
平均 266 ms，中间那次只要 120 ms，最慢一次 800 ms。用户感到的卡，往往在最慢的那一截。本章约定用**最近秩**：样本从小到大排好，百分位位置取 `n × p` 再向上取整。n=5 时，P50 取第 `ceil(5 × 0.50)=3` 个，**P50=120 ms**；P95 取第 `ceil(5 × 0.95)=5` 个，**P95=800 ms**。五个点太少，线性插值大约 680 ms，两种算法都不是 MiniShop SLA。只报平均 266 ms，会把已经很慢的那一次藏起来。

![五个样本里平均值藏不住 800 ms](assets/diagrams/ch18-p95.png)


**百分位**描述“有百分之多少的请求不超过某时间”。P50（中位数）看典型，P95 / P99 看尾部。上面这组教学样本用最近秩时 P95=800 ms；换项目时取法以约定为准。面试先能说明**为什么要看尾部**。
...
线程、连接、CPU 核也不是同一个“并发”。报告里写你实际控制的是哪一个旋钮。响应时间的 P95 是另一笔账：18.2 那五个样本用最近秩时 **P95=800 ms**，讲的是尾部有多慢，不是这里的并发人数。
...
官方入门树可以有多个 Sampler。仓库这份教学骨架只有一枪，和 `.jmx` 同构：

```text
Test Plan
└── Thread Group    threads=1, ramp-up=1s, loops=1
    ├── HTTP Request  GET /api/products
    └── Listener      View Results Tree / 写 jtl 文件
```
...
五个样本 100、110、120、200、800 ms。平均值、中间值和最大值各是多少？若只汇报平均值，会漏掉什么？按本章最近秩（排序后第 `ceil(n × p)` 个），P95 是多少？
...
2. 平均 266.0 ms，中间值 120 ms，最大 800 ms。只报平均会把已经很慢的 800 ms 藏起来。最近秩 P50=120 ms、P95=800 ms（n=5 时 5×0.95=4.75，向上取整取第 5 个），不要把平均 266 当成「大家都还行」。
...
响应时间教学样本的平均值、中间值和最大值已用 Python 3.14.3 计算：`266.0`、`120`、`800`。最近秩 P95（n=5，位置 `ceil(5×0.95)=5`）为 800。吞吐示例 `1000/50=20` 已核对。
