...
安全相关规则应由产品、安全和技术人员共同确认，测试人员不能凭经验擅自决定阈值。v1.0 **没有**登录锁定、禁用、未激活；这些澄清只练评审，不要写进项目已确认规则。
...
### 第四步：打开 PRD，看 v1.0 已经关掉哪些问题

`R-CART` / `R-CART-10` 已经确认：已登录用户改数量须为正整数，且不得超过该 SKU **当前**可售库存；教学库存 10 时 `qty=10` 允许、`qty=11` 拒绝。没有总价、没有上限 99、没有原型 20。上面 v0.1 里「待确认」的上限冲突，在正式项目里**已经关闭**。失败提示文案、失败后库里是否保持原值，PRD 仍未写——继续记待确认，不要发明。
...
1. 写明评审对象和版本（教学草案 **或** PRD v1.0）；
2. 至少 5 条问题，每条能标成歧义 / 遗漏 / 矛盾 / 不可测试；
3. 七类场景里至少覆盖功能、异常、边界、权限四类；
