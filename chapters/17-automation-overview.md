...
> **一句话核心：** 越靠近 UI 的观察越贵；分层是为了把钱花在值得重复的判定上。
...
第 16 章的 pytest 对 `POST /api/cart/items` 的 `qty=11` 断言 400，全绿。产品经理打开浏览器：登录按钮没有绑定点击，购物车提示文案是英文。接口契约对了，用户路径没过。
...
![金字塔：下面多、上面少](assets/diagrams/ch17-pyramid.png)
...
![店门一改装修，UI 脚本就要跟着改](assets/diagrams/ch17-ui-cost.png)
...
示例：MiniShop 数量规则用 pytest 接口跑 8 组 parametrize，登录按钮用一条 UI 冒烟。  
边界：不是法定比例；倒置后会又慢又脆。
