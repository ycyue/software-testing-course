...
当你在浏览器里输入网址并看到页面时，背后并不是“浏览器打开了一个文件”这么简单。仓库 MiniShop 本机地址是 `http://127.0.0.1:8765/`，未登录时第一帧是登录/注册页，不是商品首页。
...
仓库 MiniShop 本机请打开登录页：

```text
http://127.0.0.1:8765/
```

未登录时 `/` 就是登录和注册，不是商品列表。下面这条是**教学 URL**，只供拆 scheme、host、port、path、query、fragment，**不是**仓库地址，本机通常也打不开：

```text
https://shop.example.test:8443/products?keyword=mouse&page=2#reviews
```

教学示例按下 Enter 后，结果可能是商品列表，也可能是白屏、加载动画、错误页或“找不到服务器”。这些现象背后的故障位置并不相同。
...
以浏览器首次打开一个网页为例，可以拆成下面几步。仓库 MiniShop 请用 `http://127.0.0.1:8765/`；教学 URL 里的商品页也走同一条链路。
...
继续拆解上面的教学 URL（不是仓库 MiniShop；本机登录页是 `http://127.0.0.1:8765/`）：
...
仓库 MiniShop 本机没有域名，登录页就是：

```text
http://127.0.0.1:8765/
```

host 直接是 IP `127.0.0.1`，端口 `8765`。下面这条是**教学 URL**，用来说明「有域名时要先做 DNS」，**不是**仓库测试环境地址：

```text
https://shop.example.test
```

如果测试机无法正确解析该教学主机名，浏览器可能在发送业务请求之前就失败。此时反复修改登录密码没有意义。
...
类型：📖 书面实操（编号 7-1）。观察 MiniShop 登录页 `http://127.0.0.1:8765/`（未登录时 `/` 就是登录/注册，不要虚构商品列表）。
...
第一帧应看到登录和注册表单，可对照 [`assets/01-login.png`](assets/01-login.png)。若项目尚未运行，可打开练习页 [`assets/07-html-lab.html`](assets/07-html-lab.html)，或对任意已获授权的练习站点执行同样观察，但不要把第三方网站当作攻击练习目标。
...
1. 浏览器提示无法解析主机名；
2. 页面文字存在，但所有样式消失；
3. 登录按钮点了没反应（在登录页、lab 页或 combo 教学页上观察；v1.0 首页没有「加入购物车」按钮）；
4. 登录失败提示出现了，但你还说不清是前端文案还是服务没理睬。
