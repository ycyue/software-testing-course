...
- CSS 会参与样式计算和布局；
- JavaScript 可以读取和修改 DOM，也可能触发新的请求与页面更新；
...
选择 MiniShop 的商品列表页。若项目尚未运行，可对任意已获授权的练习站点执行同样观察，但不要把第三方网站当作攻击练习目标。
...
| 页面观察 | 更可能涉及 | 仍需确认 |
| --- | --- | --- |
| 商品标题 | HTML/数据呈现 | 数据来自初始 HTML 还是接口 |
| 卡片布局 | CSS | 不同窗口宽度是否变化 |
| 加购按钮 | HTML + JavaScript | 是否请求后端、失败如何提示 |
