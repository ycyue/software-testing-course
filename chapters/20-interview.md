...
> 我是零基础学测试的候选人。我会按需求写用例、提缺陷，能用 DevTools、SQL 和接口做核对。个人项目 MiniShop v1.0 覆盖注册、登录、购物车库存和创建订单，pytest 有 37 通过、1 条预期失败对应空搜索缺陷。我没有公司实习，项目是自己的练习仓库，不是某电商的核心系统。希望做功能测试，并在团队里继续补接口自动化。
...
示例：注册 201 无 token；PRD 允许 qty=10、拒绝 11；创建订单只返回 id；用户 B 读用户 A 的订单 403；空搜索与需求不符（BUG-001）。  
...
追问“不可测试的需求”：结论是缺少可观察结果或条件。原理：没有可核对的预期就无法判定通过。场景：评审搜索规则时。示例：“搜索要智能、体验好”没有口径。边界：可先要求改写成可验证语句。
...
场景：注册、登录、合法改数量、创建订单。  
示例：`TC-REG-001`、`TC-LOGIN-001`、`TC-CART-002`（qty=10）、`TC-ORDER-001` 在 `docs/test-cases.md` 标为 P0。  
...
幂等：结论是多次相同请求的效果与一次相同。原理：创建类 POST 默认会新增资源。场景：连续点两次下单。示例：v1.0 两次 `POST /api/orders` 两个 id，符合创建默认不幂等。边界：若以后需求要求幂等，期望要改。
...
| 库存规则？ | 正整数且 ≤ 当前库存，等于允许 | PRD R-CART-10 | 早期教学服务不是 v1.0 |
| 有什么 Bug？ | 空搜索返回全量 | BUG-001 | 仍开放，不写成已修复 |
| 自动化多少条？ | 以你最近一次 pytest 为准 | 审查为 37 passed / 1 xfailed | 条数以本机输出为准 |
