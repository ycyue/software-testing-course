...
> 我是零基础学测试的候选人。我会按需求写用例、提缺陷，能用 DevTools、SQL 和接口做核对。个人项目 MiniShop v1.0 覆盖登录、购物车库存和创建订单，pytest 有 22 通过、1 条预期失败对应空搜索缺陷。我没有公司实习，项目是自己的练习仓库，不是某电商的核心系统。希望做功能测试，并在团队里继续补接口自动化。
...
场景：MiniShop pytest 全绿仍有 BUG-001。  
...
原理：接口对了，按钮也可能没绑事件。  
场景：MiniShop 登录页。  
示例：label 与密码框、错误密码提示；管理员页普通用户接口 403。  
...
7. 已写断言通过 22 条；1 条按 PRD 失败并标记 xfail，对应已知缺陷，不是“没有 Bug”。
