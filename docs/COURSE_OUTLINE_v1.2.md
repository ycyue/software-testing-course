...
脊柱公式：**测试 = 观察 + 判定 + 证据**。可靠测试再补上范围和风险说明。各章是在扩展观察通道、写清判定、或留下证据。阅读顺序与实操分层见 `docs/LEARNING.md`。
...
不增加新主题。下列章拆成上/下，原文件名保留为索引：

- 第 4 章：04A 需求分析 / 04B MiniShop 评审
- 第 6 章：06A 缺陷 / 06B 测试管理
- 第 8 章：08A 页面表单 / 08B 登录态与权限
- 第 12 章：12A 查询 / 12B 写操作与核对
- 第 15 章：15A 语法 / 15B 文件与 JSON
- 第 16 章：16A pytest 基础 / 16B fixture 与项目套件

阶段测验：`chapters/quizzes/`，按学习顺序穿插。
